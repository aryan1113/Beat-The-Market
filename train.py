...
df = pd.read_csv("data/STOCK A/A.csv", 
    na_values=['null'], 
    index_col='Date', 
    parse_dates=True, 
...
''' Some Financial numbers to better help '''
...
print("Sharpe Ratio for chosen stock for chosen stock is :",
    sharpe_ratio_predicted*(252**0.5)
    )
...
print("Sortino ratio for chosen stock for chosen stock is :",
    sortino_ratio_predicted
    )
...
print("Treynor Ratio for chosen stock for chosen stock is :",
    treynor_ratio_predicted
    )

cumulative_returns_predicted = np.cumprod(1 + predicted_returns)[-1]
cumulative_returns_actual = np.cumprod(1 + actual_returns)[-1]

print("Cumulative returns are : ",
    cumulative_returns_predicted
    )
...
print("Max Drawdown for chosen stock for chosen stock is :",
...
print("Highest actual return for chosen stock for chosen stock is :",
    highest_return_predicted
    )